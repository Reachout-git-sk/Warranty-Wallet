...
async function sendWarrantyEmail(toEmail, expiredDocs, soonDocs) {
  const expiredList = expiredDocs
    .map(
      (d) =>
        `<li><strong>${d.productName}</strong> (${d.brand}) — Expired</li>`
    )
    .join("");
  ...
  const soonList = soonDocs
    .map(
      (d) =>
        `<li><strong>${d.productName}</strong> (${d.brand}) — ${d.daysLeft} days left</li>`
    )
    .join("");
...
export { startReminderJob, checkWarranties };